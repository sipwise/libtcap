#include <stdarg.h>
#include <stddef.h>
#include <setjmp.h>
#include <cmocka.h>

#include "libtcap.h"

const char tcap[] = "";
const int tcap_len = sizeof(tcap) - 1;
struct output_buffer out;
char buf[256];

static void test_localValue(void **state) {
    int i;
	OUTPUT_BUFFER_INIT(&out, buf);

	i = tcap_extract(tcap, tcap_len, "end.components.1.invoke.opCode.localValue", &out);
    assert_int_equal(i, 0);
    assert_int_equal(out.used, 2);
    out.buf[2] = '\0';
    assert_string_equal(out.buf, "20");
}

static void test_cutAndPaste(void **state) {
    int i;
	OUTPUT_BUFFER_INIT(&out, buf);

	i = inap_extract(tcap, tcap_len, "ConnectArg.cutAndPaste", &out);
    assert_int_equal(i, 0);
    assert_int_equal(out.used, 1);
    out.buf[1] = '\0';
    assert_string_equal(out.buf, "9");
}

static void test_destinationRoutingAddress(void **state) {
    int i;
	OUTPUT_BUFFER_INIT(&out, buf);

	i = inap_extract(tcap, tcap_len, "ConnectArg.destinationRoutingAddress.0", &out);
    assert_int_equal(i, 0);
    assert_int_equal(out.used, 35);
    out.buf[35] = '\0';
    assert_string_equal(out.buf, "83 90 89 10 10 80 22 08 00 55 50 05");
}

int main(void) {
    const UnitTest tests[] = {
        unit_test(test_localValue),
        unit_test(test_cutAndPaste),
        unit_test(test_destinationRoutingAddress)
    };
    return run_tests(tests);
}
